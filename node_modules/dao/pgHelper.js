...
PG.prototype.ss = function(tablename,fields,returnfields,cb){
    if(!tablename) return;
    var returnStr = "";
    //如果返回字段为空
    if(returnfields.length == 0)
...
PG.prototype.selectprovince_gdpByCode =function(tablename,fields,returnfields,cb){
    if(!tablename) return;
    var returnStr = "";
    //如果返回字段为空
    if(returnfields.length == 0)
    ...
    if(fields.length == 0) 
    	...
    	//否则根据条件字段进行查询
        //str = "select "+returnStr+ " from "+tablename+" where ";
        str = "select "+returnStr+ " from "+tablename+" where pro_code = '"+ fields+"' order by datayear";
    //console.log("strsql...:"+str);
    //console.log("value...:"+value);
    clientHelper(str,'',cb);
}
